  ...
  bool distributions::setup_distribution() {
    ...
    if ( auto *dist = std::get_if< next_distribution >( &dist_sing ) ) {
      if ( dist->distribution_time.sec_since_epoch() <=
        ...
        if ( std::distance( voters.get_table().begin(),
                            voters.get_table().end() ) > 0 ) {
          // jump into to the next distribution if there is not voters to reward
          set_next_distribution( dist->distribution_time );
...
void distributions::set_next_distribution(
    eosio::time_point_sec prev_distribution_time ) {
  distribution_sing.set(
      next_distribution{ .distribution_time =
                             prev_distribution_time + eosio::days( 1 ) },
      contract );
}
